...
function xslider(opt){
	...
	$(window).resize(() => items.css("width", `${sliderWrap.width() || sliderWrap.innerWidth}px`));	
...
$(function(){
	...
	if($(".hreviews__wrap").length){
		...
		xslider(sliderOpt);
		$(window).resize((e)=>xslider(sliderOpt));
	}
	
	
	//Animate numbers
	if($(".hfaq__stat").length){
		const csns = $.animateNumber.numberStepFactories.separator(' ');
		$('#num-since').animateNumber({ number: parseInt($('#num-since').data('number')), numberStep: csns }, 1500);
		$('#num-listed').animateNumber({ number: parseInt($('#num-listed').data('number')), numberStep: csns }, 2500);
		$('#num-feed').animateNumber({ number: parseInt($('#num-feed').data('number')), numberStep: csns }, 1700);
		$('#num-orders').animateNumber({ number: parseInt($('#num-orders').data('number')), numberStep: csns }, 1200);
		$('#num-in7d').animateNumber({ number: parseInt($('#num-in7d').data('number')), numberStep: csns }, 1900);
		$('#num-emp').animateNumber({ number: parseInt($('#num-emp').data('number')), numberStep: csns }, 1500);
	}
